...
static int packet_queue_put_private(PacketQueue *q, AVPacket *pkt)
{
    ...
    if (!pkt1)
        return -1;
    pkt1->pkt = *pkt; //拷贝AVPacket(浅拷贝，AVPacket.data等内存并没有拷贝，只是将内容指向节点的AVPacket)
    pkt1->next = NULL;
...
static int packet_queue_put(PacketQueue *q, AVPacket *pkt)
{
    ...
    ret = packet_queue_put_private(q, pkt); //真正放入实现
...
static void video_image_display(VideoState *is)
{
    ...
    vp = frame_queue_peek_last(&is->pictq); //获取需当前帧
    if (is->subtitle_st)
    {
    ...
    if (!vp->uploaded)
    {
        if (upload_texture(&is->vid_texture, vp->frame, &is->img_convert_ctx) < 0) //更新视频texture
            return;
        vp->uploaded = 1;
    ...
    SDL_RenderCopyEx(renderer, is->vid_texture, NULL, &rect, 0, NULL, vp->flip_v ? SDL_FLIP_VERTICAL : 0); //送显
...
{
    if (!is->width)
        video_open(is); //打开一个sdl窗口

    SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255); //设置图层初始颜色
    SDL_RenderClear(renderer);                      //清空渲染目标
    if (is->audio_st && is->show_mode != SHOW_MODE_VIDEO)
        video_audio_display(is);
    else if (is->video_st)
        video_image_display(is); //图像数据渲染
...
static double vp_duration(VideoState *is, Frame *vp, Frame *nextvp)
{
    ...
    {
        double duration = nextvp->pts - vp->pts;//两帧的pts间隔
        if (isnan(duration) || //duration异常情况vp->pts大，这一般在第一帧时，vp = nextvp
        duration <= 0 || //nextvp->pts 没有比 
        duration > is->max_frame_duration)//超过了间隔最大帧范围
            return vp->duration;//返回vp帧的duration
        else
            return duration;//使用两帧间的差值
    }
    else
    {
        return 0.0;//序列不同就返回0，seek后
    }
}
...
static int queue_picture(VideoState *is, AVFrame *src_frame, double pts, double duration, int64_t pos, int serial)
{
    ...
    if (!(vp = frame_queue_peek_writable(&is->pictq))) //判断是否有空间可以写入
        return -1;
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
    ...
    while (len > 0)//循环发送，直到发够所需数据长度
    {
        ...
        if (len1 > len)
            len1 = len;
        //根据audio_volume决定如何输出audio_buf,如果不进行音量调节，则可直接将音频数据拷贝进入
...
static int stream_component_open(VideoState *is, int stream_index)
{
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        decoder_init(&is->viddec, avctx, &is->videoq, is->continue_read_thread);//初始化对应的解码线程
        if ((ret = decoder_start(&is->viddec, video_thread, "video_decoder", is)) < 0)// 正式开启解码线程。
            goto out;
...
static int read_thread(void *arg)
{
    ...
    if (show_status)
        av_dump_format(ic, 0, is->filename, 0); //显示当前音视频格式数据
    ...
    {
        stream_component_open(is, st_index[AVMEDIA_TYPE_AUDIO]); //打开音频解码器
    }

    ret = -1;
    if (st_index[AVMEDIA_TYPE_VIDEO] >= 0)
    {
        ret = stream_component_open(is, st_index[AVMEDIA_TYPE_VIDEO]); //打开视频解码器
    }
    ...
    ret = 0;
    //线程关闭的时候。会清空AVFormatContext 和锁资源
fail:
    if (ic && !is->ic)
...
static void refresh_loop_wait_event(VideoState *is, SDL_Event *event)
{
    ...
    //取出事件
    SDL_PumpEvents();
    //从设备收集所有挂起的输入信息并将其放入事件队列中
   
    while (!SDL_PeepEvents(event, 1, SDL_GETEVENT, SDL_FIRSTEVENT, SDL_LASTEVENT)) {
        //是否显示鼠标
        //检查事件队列中的消息，如果没有消息则显示当前帧
        ...
        if (is->show_mode != SHOW_MODE_NONE && (!is->paused || is->force_refresh))
            video_refresh(is, &remaining_time); //立即显示当前帧视频，或者延迟remaining_time后显示
        SDL_PumpEvents();                       //继续下一个事件循环
    }
}
...
static void event_loop(VideoState *cur_stream)
{
    ...
    {
        double x;
        //当sdl event为空的时候，则在while循环中播放视频帧。否则从队列头部取一个event，退出当前函数，在上级函数中处理event
...
//注册flush packet 只是一个标记作用，用于packet队列中，在对packet队列分析时有说明
    ...
    is = stream_open(input_filename, file_iformat); //打开解码器
    if (!is)
    {
